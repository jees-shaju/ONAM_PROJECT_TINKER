...
import { Sparkles, MapPin, Clock, Flower2, Calendar, Compass } from 'lucide-react';
import { sound } from '../utils/sound';

export function Navbar() {
  const { completedMemories, formattedTimeRemaining, currentLocation, getNextRecommendation, setActiveJourney } = useApp();
  ...
  return (
    <header className="sticky top-0 z-40 w-full glass-panel border-b border-gold-500/20 px-4 lg:px-8 py-3 transition-all duration-300">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        {/* Status Indicators & Surprise Me */}
        <div className="flex items-center gap-3">
          
          {/* Location Badge */}
          <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-emerald-900/50 border border-emerald-700/40 text-xs text-cream-100">
            <MapPin className="w-3.5 h-3.5 text-gold-400 animate-pulse" />
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-gradient-to-r from-amber-500 to-terracotta-500 hover:from-gold-400 hover:to-amber-500 text-emerald-950 text-xs font-extrabold shadow-lg hover:shadow-gold-500/30 hover:scale-105 transition-all"
          >
            <Sparkles className="w-3.5 h-3.5 text-emerald-950 animate-bounce-gentle" />
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 glass-panel border-t border-gold-500/20 px-2 py-2 flex items-center justify-around">
        {navLinks.map((link) => {
          const isActive = location.pathname === link.path;
          return (
            <Link
            ...
            >
              <span>{link.label === 'Home' ? '👑' : link.label === 'Discover' ? '🌼' : link.label === 'My Day' ? '📋' : link.label === 'Map' ? '🗺️' : '🌺'}</span>
